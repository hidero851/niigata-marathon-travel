...
以下に該当する場合は、**BUGS.md に追記すること**。

| 条件 | 例 |
|------|-----|
| バグを修正したとき | スマホで表示されない、データが消えた など |
| 「なぜそう書いたか」が重要なコードを追加したとき | BOM除去処理、version===0チェック など |
| 同じミスを繰り返しそうな注意事項が出たとき | Vercel への貼り付け手順 など |

更新後は Google Drive「marathon-navi バックアップ」の BUGS.md も上書きアップロードすること。

---

### CLAUDE.md を更新する条件

以下に該当する場合は、**CLAUDE.md の該当箇所を更新すること**。

| 条件 | 例 |
|------|-----|
| 「やってはいけないこと」が新たに判明したとき | draft フラグを触ってはならない など |
| よく使うコマンドが変わったとき | スクリプトの実行方法が変わった など |
| フォルダ構成が大きく変わったとき | 新しいページやユーティリティを追加した など |
| バックアップ場所・方法が変わったとき | Google Drive のフォルダ名が変わった など |
