...
| `import` | インポート | FileSpreadsheet |
| `data` | データ確認 | BarChart2 |
...
すべての大会（静的データ + 管理画面登録）をリスト表示。

| 操作 | 対象 | 内容 |
|------|------|------|
| 編集（Pencil） | 管理画面登録大会のみ | フォームで全項目を編集 |
| 日付編集（Calendar） | すべての大会 | 開催日だけ素早く変更 |
| 公開/下書き切替（Eye） | 管理画面登録大会のみ | `draft: true/false` を切替 |
...
> 新規登録は常に **下書き（draft: true）** として保存される。公開は一覧の「公開/下書き切替」ボタンから行う。

#### 下段: 大会別サブタブ

大会を選択すると以下の7サブタブが表示される。
...
| 開催日 | date input（`YYYY-MM-DD`）。日本語フォーマットに自動変換して表示 |
| ヒーロー画像URL | 画像プレビュー付き |
| ヒーロー画像 Alt テキスト | |
| キャッチコピー | |
| サブタイトル | |
...
### 4.6 データインポートタブ (`import`)

Excel（XLSX）ファイルから大会データを一括インポートする機能。
